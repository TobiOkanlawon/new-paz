...
const Savings = () => {
    ...
    const topCards = [
        {img: '/soloUser.png', header: 'PAZ Solo Saver', text: 'Save money regularly in a locked plan with interest of up to 12% per annum.', href: '/soloSaver', color: ' #3475DF0D'},
        {img: '/familyVaultCard.png', header: 'PAZ Family Vault', text: 'Save money together with your loved ones and get interests of up to 16% per annum.', href: '/familyVault', color: '#243D7D1A'},
    ]
    const familyForm = (
        <form className={styles.modalContainer}>
            ...
            <div className={styles.addFam}>
                <input type="text" id='addFam' placeholder='Enter family member account name' />
                <button><LuCopy/> Copy link</button>
            </div>
            <label className={styles.preferredAmountLabel}>Select preferred amount</label>
            <div className={styles.preferredAmount}>
                ...
                <input type="number" name="preferredAmount" id="preferredAmount" placeholder='specify amount' />
            </div>
            ...
            <label className={styles.savingsDurationLabel}>Select savings duration</label>
            <div className={styles.savingsDuration}>
                <button>6 months</button>
                <button>1 year</button>
                <button>2 years</button>
                <button>5 years</button>
                <input type="number" name="preferredAmount" id="preferredAmount" placeholder='Specify Duration' />
            </div>
    ...
    const targetForm = (
        <form className={styles.modalContainer}>
            ...
            <div className={styles.famNameContainer}>
                <input type="text" id="famName" placeholder='What are you saving towards'/>
            </div>
            <label htmlFor="addFam">Savings Description*</label>
            <div className={styles.addFam}>
                <input type="text" id='addFam' style={{width: '100%'}} placeholder='Tell us about your purpose of this savings' />
            </div>
            <label className={styles.preferredAmountLabel}>Select preferred amount</label>
            <div className={styles.preferredAmount}>
                <button value={'5000'}>5,000</button>
  ...
  return (
    <div className={styles.container}>
        <SavingsAlert isActive={savingsAlert} isSuccessful={true} onClose={()=>{setSavingsAlert(false)}} message='Top Up Successful' />
        ...
        <div className={styles.saversContainer}>
            <div className={styles.topCardSet}>
                {
                    topCards.map((card) => (
                        <div className={styles.cardTypes} key={card.href} style={{backgroundColor: card.color}}>
                            <Image 
                            src={card.img} 
                            alt={card.header}
                            width={100}
            ...
            <div className={styles.cardTypes} style={{backgroundColor: '#3475DF0D'}}>
                <Image 
        ...
        </div>
        {
            isActive && (
                ...
                children={modalType === 'family' ? familyForm : modalType === 'target' ? targetForm : null}/>
            )
        }
    </div>
  )
